...
///
template <typename T>
class Scalar3DUpdater5 : public VCUpdater
{
...
private:
  ...
  T interpolateF2C(const Scalar3D<T> &f, int I, int J, int K)
  {
    ...
    int j = 2 * J;
    int k = 2 * K;
    return 0.125 * (f(i, j, k) + f(i + 1, j, k) + f(i, j + 1, k) + f(i + 1, j + 1, k) + f(i, j, k + 1) + f(i + 1, j, k + 1) + f(i, j + 1, k + 1) + f(i + 1, j + 1, k + 1));
  }
  ...
  T interpolateF2C(const T *fData, const Index3DS &fIndex, int I, int J, int K)
  {
    ...
    int j = 2 * J;
    int k = 2 * K;
    return 0.125 * (fData[fIndex(i, j, k)] + fData[fIndex(i + 1, j, k)] + fData[fIndex(i, j + 1, k)] + fData[fIndex(i + 1, j + 1, k)] + fData[fIndex(i, j, k + 1)] + fData[fIndex(i + 1, j, k + 1)] + fData[fIndex(i, j + 1, k + 1)] + fData[fIndex(i + 1, j + 1, k + 1)]);
  }
  ...
  T interpolateC2F(const Scalar3D<T> &c, int i, int j, int k)
  {
    ...
    return (1.0 - t) * ((1.0 - s) * ((1.0 - r) * c(I, J, K) + r * c(I + 1, J, K)) + s * ((1.0 - r) * c(I, J + 1, K) + r * c(I + 1, J + 1, K))) + t * ((1.0 - s) * ((1.0 - r) * c(I, J, K + 1) + r * c(I + 1, J, K + 1)) + s * ((1.0 - r) * c(I, J + 1, K + 1) + r * c(I + 1, J + 1, K + 1)));
  }
  ...
  T interpolateC2F(const T *cData, const Index3DS &cIndex, int i, int j, int k)
  {
    ...
    return (1.0 - t) * ((1.0 - s) * ((1.0 - r) * cData[cIndex(I, J, K)] + r * cData[cIndex(I + 1, J, K)]) + s * ((1.0 - r) * cData[cIndex(I, J + 1, K)] + r * cData[cIndex(I + 1, J + 1, K)])) + t * ((1.0 - s) * ((1.0 - r) * cData[cIndex(I, J, K + 1)] + r * cData[cIndex(I + 1, J, K + 1)]) + s * ((1.0 - r) * cData[cIndex(I, J + 1, K + 1)] + r * cData[cIndex(I + 1, J + 1, K + 1)]));
  }

  //FEAST.s 勾配計算
  T CalcGradient(const Scalar3D<T> &c, int I, int J, int K,
                 int i_shift, int j_shift, int k_shift)
  {

    T dp = (c(I, J, K) -
            c(I + i_shift, J + j_shift, K + k_shift)) *
           0.5;
  ...
  //FEAST.s 勾配計算
  T CalcGradient(const T *cData, const Index3DS &cIndex, int I, int J, int K,
                 int i_shift, int j_shift, int k_shift)
  {

    T dp = (cData[cIndex(I, J, K)] -
            cData[cIndex(I + i_shift, J + j_shift, K + k_shift)]) *
           0.5;
  ...
  //勾配での補間
  T Gradient(const T data, const T dp)
  {
...
#pragma omp parallel for if (nz >= 16)
      for (int k = 0; k < nz; ++k)
      {
        for (int j = 0; j < ny; ++j)
        {
          for (int i = 0; i < vc; ++i)
          {
            fData[fIndex(-1 - i, j, k)] = fData[fIndex(0 - i, j, k)] + buffer[i + vc * j + (vc * ny) * k];
          }
        }
      }
      break;
...
#pragma omp parallel for if (nz >= 16)
      for (int k = 0; k < nz; ++k)
      {
        for (int j = 0; j < ny; ++j)
        {
          ...
          {
            fData[fIndex(i, j, k)] = fData[fIndex(i - 1, j, k)] + buffer[i - nx + vc * j + (vc * ny) * k];
          }
        }
      }
      break;
...
#pragma omp parallel for if (nz >= 16)
      for (int k = 0; k < nz; ++k)
      {
        for (int j = 0; j < vc; ++j)
        {
          for (int i = 0; i < nx; ++i)
          {
            fData[fIndex(i, -1 - j, k)] = fData[fIndex(i, 0 - j, k)] + buffer[i + nx * j + (nx * vc) * k];
          }
        }
      }
      break;
...
#pragma omp parallel for if (nz >= 16)
      for (int k = 0; k < nz; ++k)
      {
        for (int j = ny; j < ny + vc; ++j)
        {
          for (int i = 0; i < nx; ++i)
          {
            fData[fIndex(i, j, k)] = fData[fIndex(i, j - 1, k)] + buffer[i + nx * (j - ny) + (nx * vc) * k];
          }
        }
      }
      break;
...
#pragma omp parallel for if (nz >= 16)
      for (int k = 0; k < vc; ++k)
      {
        for (int j = 0; j < ny; ++j)
        {
          for (int i = 0; i < nx; ++i)
          {
            fData[fIndex(i, j, -1 - k)] = fData[fIndex(i, j, 0 - k)] + buffer[i + nx * j + (nx * ny) * k];
          }
        }
      }
      break;
...
#pragma omp parallel for if (nz >= 16)
      for (int k = nz; k < nz + vc; ++k)
      {
        for (int j = 0; j < ny; ++j)
        {
          for (int i = 0; i < nx; ++i)
          {
            fData[fIndex(i, j, k)] = fData[fIndex(i, j, k - 1)] + buffer[i + nx * j + (nx * ny) * (k - nz)];
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          for (int I = 0; I < vc; I++)
          {
            T dp = CalcGradient(cData, cIndex, nx - 1, J / 2 + J0 / 2, K / 2 + K0 / 2, 1, 0, 0);
            fData[fIndex(-1 - I, J, K)] = Gradient(fData[fIndex(0 - I, J, K)], dp);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          for (int I = 0; I < vc; I++)
          {
            T dp = CalcGradient(cData, cIndex, 0, J / 2 + J0 / 2, K / 2 + K0 / 2, -1, 0, 0);
            fData[fIndex(nx + I, J, K)] = Gradient(fData[fIndex(nx - 1 + I, J, K)], dp);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < vc; J++)
        {
          for (int I = 0; I < nx; I++)
          {
            T dp = CalcGradient(cData, cIndex, I / 2 + I0 / 2, ny - 1, K / 2 + K0 / 2, 0, 1, 0);
            fData[fIndex(I, -1 - J, K)] = Gradient(fData[fIndex(I, 0 - J, K)], dp);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < vc; J++)
        {
          for (int I = 0; I < nx; I++)
          {
            T dp = CalcGradient(cData, cIndex, I / 2 + I0 / 2, 0, K / 2 + K0 / 2, 0, -1, 0);
            fData[fIndex(I, ny + J, K)] = Gradient(fData[fIndex(I, ny - 1 + J, K)], dp);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < vc; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          for (int I = 0; I < nx; I++)
          {
            T dp = CalcGradient(cData, cIndex, I / 2 + I0 / 2, J / 2 + J0 / 2, nz - 1, 0, 0, 1);
            fData[fIndex(I, J, -1 - K)] = Gradient(fData[fIndex(I, J, 0 - K)], dp);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < vc; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          for (int I = 0; I < nx; I++)
          {
            T dp = CalcGradient(cData, cIndex, I / 2 + I0 / 2, J / 2 + J0 / 2, 0, 0, 0, -1);
            fData[fIndex(I, J, nz + K)] = Gradient(fData[fIndex(I, J, nz - 1 + K)], dp);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          ...
          {
            int m = I + vc * (J + ny * K);
            buffer[m] = CalcGradient(cData, cIndex, 0, J / 2 + J0 / 2, K / 2 + K0 / 2, -1, 0, 0);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          ...
          {
            int m = I + vc * (J + ny * K);
            buffer[m] = CalcGradient(cData, cIndex, nx - 1, J / 2 + J0 / 2, K / 2 + K0 / 2, 1, 0, 0);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < vc; J++)
        {
          ...
          {
            int m = I + nx * (J + vc * K);
            buffer[m] = CalcGradient(cData, cIndex, I / 2 + I0 / 2, 0, K / 2 + K0 / 2, 0, -1, 0);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < nz; K++)
      {
        for (int J = 0; J < vc; J++)
        {
          ...
          {
            int m = I + nx * (J + vc * K);
            buffer[m] = CalcGradient(cData, cIndex, I / 2 + I0 / 2, ny - 1, K / 2 + K0 / 2, 0, 1, 0);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < vc; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          ...
          {
            int m = I + nx * (J + ny * K);
            buffer[m] = CalcGradient(cData, cIndex, I / 2 + I0 / 2, J / 2 + J0 / 2, 0, 0, 0, -1);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int K = 0; K < vc; K++)
      {
        for (int J = 0; J < ny; J++)
        {
          ...
          {
            int m = I + nx * (J + ny * K);
            buffer[m] = CalcGradient(cData, cIndex, I / 2 + I0 / 2, J / 2 + J0 / 2, nz - 1, 0, 0, 1);
          }
        }
      }
      break;
