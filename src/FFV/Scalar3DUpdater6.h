...
///
/// @file Scalar3DUpdater6.h
/// @brief スカラデータクラス仮想セルアップデータ
///

#ifndef SCALAR_3D_UPDATER6_H
#define SCALAR_3D_UPDATER6_H
...
///
template <typename T>
class Scalar3DUpdater6 : public VCUpdater {
...
public:
  ...
  ///
  Scalar3DUpdater6(const NeighborInfo* neighborInfo,
...
*/
  ...
  T interpolateF2C(const T* fData, const Index3DS& fIndex, int I, int J, int K) {
    ...
    int k = 2 * K;
		if(
				fData[fIndex(i  ,j  ,k  )] == target_id ||
				fData[fIndex(i+1,j  ,k  )] == target_id ||
				fData[fIndex(i  ,j+1,k  )] == target_id ||
				fData[fIndex(i  ,j  ,k+1)] == target_id ||
				fData[fIndex(i  ,j+1,k+1)] == target_id ||
				fData[fIndex(i+1,j  ,k+1)] == target_id ||
				fData[fIndex(i+1,j+1,k  )] == target_id ||
				fData[fIndex(i+1,j+1,k+1)] == target_id
			) {
			return target_id;
		} 
		return -1;
  ...
  T interpolateF2C_Y_M(const T* fData, const Index3DS& fIndex, int I, int J, int K) {
    int i = 2 * I;
    int j = 2 * J;
    int k = 2 * K;
		if(
  ...
  T interpolateF2C_Y_P(const T* fData, const Index3DS& fIndex, int I, int J, int K) {
    int i = 2 * I;
    int j = 2 * J;
    int k = 2 * K;
		if(
  ...
  T interpolateF2C_Z_M(const T* fData, const Index3DS& fIndex, int I, int J, int K) {
    int i = 2 * I;
    int j = 2 * J;
    int k = 2 * K;
		if(
  ...
  T interpolateF2C_Z_P(const T* fData, const Index3DS& fIndex, int I, int J, int K) {
    int i = 2 * I;
    int j = 2 * J;
    int k = 2 * K;
		if(
  ...
  T interpolateC2F(const T* cData, const Index3DS& cIndex, int i, int j, int k) {
		...
		if( cData[cIndex(I  ,J  ,K  )] == target_id ) {
			return target_id;
		}
		return -1;
  }
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < vc; i++) {
            cData[cIndex(i-vc, j+j0, k+k0)] = interpolateF2C_X_M(fData, fIndex, i+nx/2-vc, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < vc; i++) {
            cData[cIndex(i+nx, j+j0, k+k0)] = interpolateF2C_X_P(fData, fIndex, i, j, k);
          }
        }
      }
      break;
      ...
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < vc; j++) {
          for (int i = 0; i < nx/2; i++) {
            cData[cIndex(i+i0, j-vc, k+k0)] = interpolateF2C_Y_M(fData, fIndex, i, j+ny/2-vc, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < vc; j++) {
          for (int i = 0; i < nx/2; i++) {
            cData[cIndex(i+i0, j+ny, k+k0)] = interpolateF2C_Y_P(fData, fIndex, i, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < vc; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < nx/2; i++) {
            cData[cIndex(i+i0, j+j0, k-vc)] = interpolateF2C_Z_M(fData, fIndex, i, j, k+nz/2-vc);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < vc; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < nx/2; i++) {
            cData[cIndex(i+i0, j+j0, k+nz)] = interpolateF2C_Z_P(fData, fIndex, i, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < vc; i++) {
						int m = i + vc*(j + ny/2*k);
            buffer[m] = interpolateF2C_X_M(fData, fIndex, i, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < vc; i++) {
						int m = i + vc*(j + ny/2*k);
            buffer[m] = interpolateF2C_X_P(fData, fIndex, i+nx/2-vc, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < vc; j++) {
          for (int i = 0; i < nx/2; i++) {
						int m = i + nx/2*(j + vc*k);
            buffer[m] = interpolateF2C_Y_M(fData, fIndex, i, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < nz/2; k++) {
        for (int j = 0; j < vc; j++) {
          for (int i = 0; i < nx/2; i++) {
						int m = i + nx/2*(j + vc*k);
            buffer[m] =  interpolateF2C_Y_P(fData, fIndex, i, j+ny/2-vc, k);
          }
        }
      }
      break;
      ...
      for (int k = 0; k < vc; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < nx/2; i++) {
            ...
            buffer[m] = interpolateF2C_Z_M(fData, fIndex, i, j, k);
          }
        }
      }
      break;
...
#pragma omp parallel for collapse(3)
      for (int k = 0; k < vc; k++) {
        for (int j = 0; j < ny/2; j++) {
          for (int i = 0; i < nx/2; i++) {
						int m = i + nx/2*(j + ny/2*k);
            buffer[m] = interpolateF2C_Z_P(fData, fIndex, i, j, k+nz/2-vc);
          }
        }
      }
      break;
